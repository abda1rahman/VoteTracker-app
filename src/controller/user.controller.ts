...
export const updateEnvoyHandler = async (
  ...
  try {
    const { firstName, lastName, password, phone, ssn, newSSN } = req.body;
    ...
    const updatedEnvoy = await updateEnvoyData({
      firstName,
      lastName,
      password,
      phone,
...
export const getAllCandidateHandler = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json(successResponse(res.statusCode, "All Candidate", allCandidate));
  } catch (error: any) {
...
export const getEnvoyByCandidateIdHandler = async (
  req: Request<CandidateParamsInput>,
  res: Response
) => {
  try {
    ...
    // Get all envoy by candidate id
    const allEnvoy: EnvoyModelType[] = await getEnvoyByCandidateId(
      candidate.id
    );
    ...
    if (allEnvoy.length === 0) {
      return res
        .status(404)
        .json(errorResponse(404, "cannot find any enovy for this candidate"));
    }

    return res
      .status(200)
      .json(successResponse(200, `All enovy for candidate`, allEnvoy));
  } catch (error: any) {
    log.error(
      "Error in controller getEnvoyByCandidateIdHandler",
      error.message
    );
...
export const getAllEnvoyHandler = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json(successResponse(res.statusCode, "All Envoy", allEnvoy));
  } catch (error: any) {
