...
async function createCandidate(Data: CreateUserInput) {
  try {
    ...
    // Format data
    const candidateData = omit(
      { ...user.toJSON(), ...candidate.toJSON(), id: candidate.id.toString() },
...
async function createEnvoy(
  Data: AuthTypes.IenvoyInput
): Promise<AuthTypes.Ienvoy> {
  try {
    const {
      firstName,
      lastName,
...
async function createDeveloper(
  ...
  try {
    ...
    // Format data
    const developerData = omit(
      { ...user.toJSON(), ...developer.toJSON(), id: developer.id.toString() },
      ["_id", "password", "user_id", "createdAt", "updatedAt"]
    );
